...
namespace Beaver3D.Model
{
	...
	public class Structure
	{
		...
		{
			bool flag = this.LoadCases.Contains(LC);   //判断是否已经包含荷载的名字
			if (flag)
			{
		...
		public List<LoadCase> GetLoadCasesFromNames(List<string> LoadCaseNames)
		{
			...
			bool flag2 = LoadCaseNames[0] == "all";
			...
			{
				List<LoadCase> list = this.LoadCases.FindAll((LoadCase x) => LoadCaseNames.Contains(x.Name));
